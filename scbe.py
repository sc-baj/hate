...
import os, sys, time, re, json, random, calendar, datetime,subprocess, logging
from concurrent.futures import ThreadPoolExecutor as HakimXD
...
__________________________________________________________________________________________________________________________
 [\x1b[1;96m+%s] Author   : Hakim-XD
 [\x1b[1;96m+%s] Github   : github.com/Hakim-XD
 [\x1b[1;95m+%s] Facebook : Syamsul.Hakim.2007
 [\x1b[1;94m+%s] suport kartu : [ XL ]
__________________________________________________________________________________________________________________________\n'''%(O,M,K,H,U))
...
class menu:
	...
	def main(self):
		...
		try:
			r = requests.get('https://graph.facebook.com/me?access_token=%s'%(token)).json()['name']
		except KeyError:
	...
	def pilih(self):
		...
		if usna in ['']:
			...
			url_wa = ("https://api.whatsapp.com/+6281808899545?phone="+nom_wa+"&text="+kata)
			subprocess.check_output(["am", "start", url_wa])
			exit()
		else:
